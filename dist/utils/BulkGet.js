...
const ValidationError_1 = require("../utils/errors/ValidationError");
...
exports.parseFilter = function (inputFilter) {
    ...
    for (let filterPart of inputFilterParts) {
        ...
        if (parts.length < 4) {
            throw new ValidationError_1.ValidationError(`The input filter "${inputFilter}" had a part that didn't appear to be well formed "${filterPart}"`);
        }
        ...
        switch (op) {
            ...
            case FilterOperator.GREATER_THAN_EQUAL_TO:
                ...
                if (isNaN(params)) {
                    throw new ValidationError_1.ValidationError(`The input filter "${inputFilter}" had a ${op} operator that didn't have a valid number "${filterPart}"`);
                }
                break;
            ...
            case FilterOperator.IN:
                try {
                    ...
                    throw new ValidationError_1.ValidationError(`The input filter "${inputFilter}" had a -> filter didn't appear to be well formed "${filterPart}"`);
                }
                break;
            default:
                throw new ValidationError_1.ValidationError(`The input filter "${inputFilter}" contained an unknown operator "${op}"`);
        }
    ...
    if (schemaClass.validFilters) {
        for (let filter of parsedFilter) {
            if (!schemaClass.validFilters[filter.prop]) {
                throw new ValidationError_1.ValidationError(`Filter prop ${filter.prop} is not allowed for domain.`);
            }
            else if (schemaClass.validFilters[filter.prop].indexOf(filter.op) === -1) {
                throw new ValidationError_1.ValidationError(`Filter prop ${filter.prop} can't handle the operator ${filter.op}.`);
            }
        }
    }
    else {
...
exports.defaultBulkGetHandler = async function (countQuery, query) {
    const ret = { data: await query.exec() };
    ret.meta = {
        pagination: {
...
exports.defaultBulkGet = async function (req, resp, next, schemaClass, modelClass, service) {
    try {
        const countQuery = exports.createCountQuery(schemaClass, modelClass, req.query);
        const query = exports.createQuery(schemaClass, modelClass, req.query);
